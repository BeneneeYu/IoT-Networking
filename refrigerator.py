...
class Refrigerator():
    ...
    def gen_grocery_order_msg(self):
        ...
        for i in range(1, 6, 1):
            groc_msg.add_veggies(i, round(random.uniform(2, 10), 2))
            groc_msg.add_milk(i, round(random.uniform(2, 10), 2))
            groc_msg.add_bread(i, round(random.uniform(2, 10), 2))
            groc_msg.add_meat(i, round(random.uniform(2, 10), 2))
        groc_msg.drinks = {
            "cans": {
    ...
    def gen_health_status_msg(self):
        ...
        status_msg.set_dispenser(random.choice([1, 2, 3]))
        status_msg.icemaker = random.randint(1, 100)
        status_msg.set_lightbulb(random.choice([1, 2]))
        status_msg.fridge_temp = random.randint(50, 100)
        status_msg.freezer_temp = random.randint(-40, 50)
        status_msg.set_sensor_status(random.choice([1, 2]))
        return status_msg
...
def parseCmdLineArgs():
    ...
    parser.add_argument("-g", "--groc_ip", default="10.0.0.5",
                        help="IP Address of Grocery server to connect to (default: localhost i.e., 10.0.0.5)")
    parser.add_argument("-p", "--groc_port", type=int, default=5555,
                        help="Port that grocery server is listening on (default: 5555)")
    parser.add_argument("-s", "--status_ip", default="10.0.0.6",
                        help="IP Address of Health Status server to connect to (default: localhost i.e., 10.0.0.6)")
    parser.add_argument("-q", "--status_port", type=int, default=5555,
                        help="Port that Healt Status server is listening on (default: 5555)")
    parser.add_argument("-r", "--req_ratio", default="1:1",
                        help="Ratio of grocery orders sent to health status (default: 1:1 implying equal number)")
