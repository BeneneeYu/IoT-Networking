        ...
        try:
            ...
            if normal:
                ...
                for i in range(0, int(self.chunk_num)):
                    self.send_segment(actual_packet[int(i * self.mtu_len): (int(i * self.mtu_len) + self.mtu_len)],
                                      self.mtu_len, False)
                print("All the chunks all sent...")
                self.send_segment(b'')  # to notify that all chunks are sent
        except Exception as e:
            raise e
        ...
        try:
            ...
            if normal:
                ...
                while len(appln_msg) < self.packet_size:
                    appln_msg += self.recv_segment(0, False)
