    ...
    def send_appln_msg(self, payload, size, normal=True):
        try:
            ...
            # client side
                ...
                while self.seq_num < int(self.chunk_num):
                    window = [b'', b'', b'', b'', b'', b'', b'', b'']
                    # 0...7
                    for i in range(0, self.window_size):
                        start = int((self.seq_num + i) * self.mtu_len)
                        if (start + self.mtu_len) > self.packet_size:
                            break
                        window[i] = str(i).encode('utf-8') + actual_packet[
                                                             start:min(len(actual_packet), (start + self.mtu_len))]
                    base = 0
                    check_num = 0
                    checked = [False, False, False, False, False, False, False, False]
                    for i in range(base, self.window_size):
                        if window[i] == b'':
                            break
                        self.send_segment(window[i], len(window[i]))
                    ...
                    while True:
                        ...
                        if self.nw_obj.get_socket() in socks:
                            print("has response")
                            resp = self.recv_segment()
                            head = resp.decode('utf-8')[0]
                            if head == 'A':
                                continue
                            recv_seq_num = int(head)
                            if checked[recv_seq_num] == False:
                                checked[recv_seq_num] = True
                                check_num += 1
                            if check_num == self.window_size:
                ...
                # stable transmission
                self.seq_num = 0
                self.nw_obj.send_packet(b'A', 0, True)  # to notify that all chunks are sent
        except Exception as e:
            raise e
        ...
        try:
            ...
            # server receive appln msg
                ...
                while len(appln_msg) < self.packet_size:
                    ...
                    if chunk.decode('utf-8')[0] == 'A':
                        self.send_segment('A'.encode('utf-8'))
                        continue
                    ...
                    if checked[recv_seq_num] == False:
                        checked[recv_seq_num] = True
                        check_num += 1
                        window[recv_seq_num] = chunk[1:1 + self.mtu_len]
                    self.send_segment((str(recv_seq_num)).encode('utf-8'))
                    if check_num == self.window_size:
                        checked = [False, False, False, False, False, False, False, False]
                        check_num = 0
                        base += self.window_size
                        for chunk in window:
                            appln_msg += chunk
