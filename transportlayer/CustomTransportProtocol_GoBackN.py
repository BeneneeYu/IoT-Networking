                ...
                while self.seq_num < int(self.chunk_num):
                    ...
                    for i in range(0, self.window_size):
                        ...
                        if (start + self.mtu_len) > self.packet_size:
                            break
                        self.window[i] = str(i).encode('utf-8') + actual_packet[start:min(len(actual_packet), (start + self.mtu_len))]
                    base = 0
                    for i in range(base, self.window_size):
                        self.send_segment(self.window[i], len(self.window[i]))
                    ...
                    while True:
                        ...
                        if self.nw_obj.get_socket() in socks:
                            ...
                            base = max(base, recv_seq_num)
                            if base == self.window_size - 1:
            ...
            # server receive appln msg
                ...
                while len(appln_msg) < self.packet_size:
                    ...
                    if recv_seq_num == self.seq_num:
                        ...
                        self.send_segment((str(max(0, self.seq_num - 1))).encode('utf-8'))
